...
public class SupplierGoodsController {
	...
	//增加供应信息
		...
		public String deleteSupplierInfo(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
				Model model,SupplierGoods supplierGoods,@RequestParam(value="id",required=false) String supplierGoodsIdStr
				) throws ServletException, IOException{
		...
		//搜索指定的供应信息
			...
			public String searchSupplierGoods(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
					Model model,SupplierGoods supplierGoods,@RequestParam(value="supplierGoods_search",required=false) String supplierGoods_search ) throws ServletException, IOException{
			//判断传入的是供应商ID还是商品名
			//输入ID是精确查询 输入名字是模糊查询
					if(supplierGoods_search!=""){
					char first=supplierGoods_search.trim().charAt(0);
					if(first=='1' || first=='2'|| first=='3' ||first=='4' || first=='5'|| first=='6' ||first=='7' || first=='8'|| first=='9' || first=='0'){
					int supplier_id=Integer.parseInt(supplierGoods_search);
						 
					List<SupplierGoods> supplierGoodsInfoList=supplierGoodsService.selectSupplierGoodsBySupplierId(supplier_id);
					...
					List<SupplierGoods> supplierGoodsInfoList=supplierGoodsService.selectSupplierGoodsByGoodsName(goods_name);
