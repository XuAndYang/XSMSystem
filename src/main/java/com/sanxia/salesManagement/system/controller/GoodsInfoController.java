...
//显示用户列表
	...
	public String add(HttpServletRequest req, HttpServletResponse resp, HttpSession session, Model model)
		...
		String remaining_str= req.getParameter("remaining_number");
		int remaining=Integer.parseInt(remaining_str);
//		Date create_time = new SimpleDateFormat("yyyy-MM-dd").parse(time);
	...
	//删除指定商品信息
		...
		public String deleteGoodsInfo(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
				Model model,GoodsInfo goodsInfo,@RequestParam(value="goodsId",required=false) String goodsIdStr
				) throws ServletException, IOException{
...
//			String userIdStr= req.getParameter("userId");
		...
		//搜索指定的商品信息
				...
				Model model,GoodsInfo goodsInfo,@RequestParam(value="goodsInfo_search",required=false) String goodsInfo_search ) throws ServletException, IOException{
		//判断传入的是编号还是名字
				if(goodsInfo_search!=""){
