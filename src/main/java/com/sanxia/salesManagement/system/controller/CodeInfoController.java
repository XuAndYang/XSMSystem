...
public class CodeInfoController {
	...
	//显示条形码信息列表
		...
		public String deleteCodeInfo(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
				Model model,CodeInfo codeInfo,@RequestParam(value="id",required=false) String idStr
				) throws ServletException, IOException{
		...
		public String searchCode(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
		Model model,CodeInfo codeInfo,@RequestParam(value="codeInfo_search",required=false) String codeInfo_search ) throws ServletException, IOException{
//判断传入的是编号还是名字
		if(codeInfo_search!=""){
		char first=codeInfo_search.trim().charAt(0);
		if(first=='1' || first=='2'|| first=='3' || first=='4' || first=='5'|| first=='6' || first=='7' || first=='8'|| first=='9'){
		
		int goods_id=Integer.parseInt(codeInfo_search);
		List<CodeInfo> codeInfoList=codeInfoService.selectCodeInfoByGoodsId(goods_id);
